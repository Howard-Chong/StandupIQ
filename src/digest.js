...
function buildDigestData(responses, rtsFindings = []) {
  // Deduplicate: one entry per user (latest only)
  const latest = getLatestPerUser(responses);
  const status = getTeamStatus(latest);
...
async function postDigest(app, digestData, channelId) {
  try {
    ...
    const latestResponses = getLatestPerUser(responses);
    ...
    // Calculate next standup reminder time
    const now = new Date();
    ...
    const nextLabel = nextReminder.toLocaleDateString('en-US', {
      weekday: 'long',
      month: 'short',
      day: 'numeric',
    });
    ...
    const blocks = [
      ...
      // Team status bar
      {
        ...
        text: {
          type: 'mrkdwn',
          text: `*Status:* ${status.emoji} ${status.label} — ${latestResponses.length} update(s), ${rtsFindings.length} RTS flag(s)`,
        },
      },
      { type: 'divider' },
    ];

    // Individual team member updates — one card per user, divider between
    if (latestResponses.length > 0) {
      ...
      for (let i = 0; i < latestResponses.length; i++) {
        ...
        // Add divider between users (but not after the last one)
        if (i < latestResponses.length - 1) {
    ...
    // RTS Findings section — always shown
    blocks.push({
      type: 'section',
      text: {
        type: 'mrkdwn',
        text: '*🔍 RTS Blocker Detection*',
      },
    });
    ...
    if (rtsFindings.length > 0) {
      ...
      blocks.push({
        ...
        text: {
          type: 'mrkdwn',
          text: '✅ No hidden blockers detected',
        },
      });
    }

    // Footer with next standup reminder
    blocks.push({ type: 'divider' });
    blocks.push({
      type: 'context',
      elements: [
        {
          type: 'mrkdwn',
          text: `🤖 Generated by StandupIQ • Next standup: ${nextLabel} at 9:00 AM`,
        },
      ],
    });
